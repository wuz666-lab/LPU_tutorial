...
def Reduce(x, slice_m, slice_n, reduce_m_en, reduce_n_en, reduce_mode):
    ...
    if reduce_mode == 0: 
        if reduce_n_en:
            x = torch.max(x_mn, dim=1).values
        if reduce_m_en:
            x = torch.max(x_mn, dim=0).values
    elif reduce_mode == 1:
        if reduce_n_en:
            x = torch.min(x_mn, dim=1).values
        if reduce_m_en:
            x = torch.min(x_mn, dim=0).values
    elif reduce_mode == 2:
        if reduce_n_en:
            x = torch.sum(x_mn, dim=1)
        if reduce_m_en:
            x = torch.sum(x_mn, dim=0)
    elif reduce_mode == 3:
        if reduce_n_en:
            x = torch.mean(x_mn, dim=1)
        if reduce_m_en:
            x = torch.mean(x_mn, dim=0)
    return x
